...
def basic(
    ...
    if file_name is None:
        file_name = input(
            "Please input the file_name of the data: (if it is multiset data, please seperate them"
    ...
    if type(function_name) == str:
        if function_name == "1" or function_name == "tucker_als":
            [Final, Rec] = pyten.method.tucker_als(
                X, r, omega, tol, maxiter, init, printitn
            )
            full = Final.totensor()
        elif function_name == "2" or function_name == "cp_als":
            [Final, Rec] = pyten.method.cp_als(
                X, r, omega, tol, maxiter, init, printitn
            )
            full = Final.totensor()
            ...
            X1 = [X.data]
            multi = input(
                "Please input how many other multiset files you want to couple with the first one "
                "(Input 'None' if no other info.) \n"
            )
            if multi != "None":
                for i in range(int(multi)):
                    FileName2 = input(
                        "Please input the file_name of the "
                        + str(i + 2)
                        + " slice of the multiset data:\n"
                    )
                    if FileName2 != "None":
