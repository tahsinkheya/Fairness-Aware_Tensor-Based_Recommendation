...
def dynamic(
    ...
    if type(function_name) == str:
        ...
        if function_name == "1" or function_name == "onlineCP":
            ...
            if Former_result == "1":
                ...
                # MastCore
                self1 = pyten.method.MAST(
                    init, omega_block, self.finalAs, olddims, newdims, idx, r
                )
                self1.update()
                ...
                # Reconstruction
                Rec = pyten.tenclass.Tensor(
                    (1 - Omega1.data) * full.data + X.data * Omega1.data
                )
    ...
    if save__file is None:
        ...
        if SaveOption == "1":
            save__file = input(
                "Please input the address and fileName to save the result: (end in '.npy')\n"
            )
