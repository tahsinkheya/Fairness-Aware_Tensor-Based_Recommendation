...
def auxiliary(
        ...
        if aux_mode is None:
            ...
            if aux_mode != "None":
                for i in range((len(aux_mode) + 1) / 2):
                    ...
                    FileName2 = input(
                        "Please input the file_name of the Auxiliary Similarity Matrix of Mode "
                        + str(Mode)
                        + " :\n"
                    )
                    if FileName2 != "None":
        ...
        if aux_mode is None:
            ...
            if aux_mode != "None":
                for i in range((len(aux_mode) + 1) / 2):
                    Mode = int(aux_mode[i * 2])
                    FileName2 = input(
                        "Please input the file_name of the Coupled Matrix of Mode "
                        + str(Mode)
                        + " (Input 'None' if no auxiliary info):\n"
                    )
                    print("\n")
                    if FileName2 != "None":
                        ...
                        Mat_siz = Mat_siz + 1
                        X2 = scipy.sparse.coo_matrix(
                            (Mat_vals, (Mat_subs[:, 0], Mat_subs[:, 1])),
    ...
    if type(function_name) == str:
        if function_name == "1" or function_name == "AirCP":
            ...
            [Final, Rec, V] = pyten.method.cmtf(
                X, Y, CM, r, omega, tol, maxiter, init, printitn
            )
            full = Final.totensor()
